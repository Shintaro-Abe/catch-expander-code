...
エージェントの各 step は原則として `claude -p --output-format json` で構造化レスポンスを返す `call_claude` ヘルパー方式を採用している。これはトピック解析・ワークフロー設計・リサーチャー要約・レビュアー合否などの **小さな構造化データ**で十分機能している。

ただし **2 つのケースで Claude Code CLI の Write ツール経由のファイル書き込み方式 (workspace モード) を採用**:
