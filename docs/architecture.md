...
| サブエージェント管理 | SDK `query()` の同期ファサード `_query_claude_sync`（asyncio.run、ThreadPoolExecutorで並列制御。ADR 0001） |
...
### 統一 JSON 方式とファイル書き込み方式の使い分け

エージェントの各 step は原則として Agent SDK 経由でモデル応答テキスト（フェンス付き JSON）を返す `call_claude` ヘルパー方式を採用している（2026-07 に `claude -p --output-format json` subprocess から `claude-agent-sdk` へ移行、ADR 0001。応答は `_parse_claude_response` で「dict を返すか `ClaudeResponseParseError`」の厳密契約でパース）。これはトピック解析・ワークフロー設計・リサーチャー要約・レビュアー合否などの **小さな構造化データ**で十分機能している。

ただし **2 つのケースで Claude の Write ツール経由のファイル書き込み方式 (workspace モード) を採用**:

1. **コード成果物 (`iac_code` / `program_code`)**: `call_claude_with_workspace` (2026-04-25 移行)
2. **テキスト成果物 (ジェネレーター本体)**: `call_claude_with_text_workspace` (2026-05-13 移行、`.steering/20260512-parse-claude-response-dict-contract/`)

### なぜ 2 経路を workspace モードにしたか (性質的差異)

| 出力 | サイズ | 特殊文字密度 / 構造的不安定性 | JSON 適性 | 方式 |
|------|--------|--------------|-----------|------|
| トピック解析 | ~500 字 | 低 | ◯ | `call_claude` |
| ワークフロー設計 | ~1,000 字 | 低 | ◯ | `call_claude` |
| リサーチャー要約 | ~2,000 字 | 中 | ◯ | `call_claude` |
| レビュアー合否 | ~1,000 字 | 低 | ◯ | `call_codex` (GPT-5.5) |
| Fixer (修正再生成) | ~10,000+ 字 | 中 | △ | `call_claude` (将来 workspace 検討) |
| **テキスト成果物 (ジェネレーター)** | **30,000+ 字** | **高 (LLM の Part 分割応答リスク、2026-05-12 観測)** | **✗** | **`call_claude_with_text_workspace`** |
| **コード成果物 (`iac_code` / `program_code`)** | **10,000+ 字** | **高 (HCL/Python 構文の `\"`、`${var}`、`\\n`、ヒアドキュメント等)** | **✗** | **`call_claude_with_workspace`** |

2 つの経路だけ「JSON 適性が低い」と判断した根拠が異なる:

- **コード成果物**: JSON エスケープと衝突する特殊文字を密に含む構造的非互換性。過去 5 回の修正でも parse_error 再発 → 2026-04-25 にファイル書き込み方式へ移行
- **テキスト成果物**: 30,000 字超の応答が必要なケースで Claude Sonnet 4.6 が独自に「Part 1 / Part 2 分割応答」戦略を発火 (2026-05-12 観測)。同トピック 3 回試行で 2 回失敗 (`exec-20260512100453-f253b974`、`TypeError: pop expected at most 1 argument, got 2`)。LLM の確率的判断による境界破壊 → 2026-05-13 にファイル書き込み方式へ移行
...
2. Agent SDK `query()`（`ClaudeAgentOptions(allowed_tools=["Write","Edit"], cwd=sandbox)`）で Claude を起動
...
ブラウザ管理者向けのオブザービリティダッシュボードを構成するインフラコンポーネントを整理する。

### 配信・ルーティング

| コンポーネント | 設定 | 役割 |
|--------------|------|------|
| CloudFront | `duf2tc37sx7rn.cloudfront.net` | SPA 静的配信 + `/api/*` を API Gateway HTTP API へプロキシ |
| S3 | `catch-expander-frontend-{account}` | Vite ビルド成果物（`frontend/dist/`）を格納 |
| API Gateway HTTP API | `AWS::Serverless::HttpApi` | `/api/v1/*` エンドポイント群を Lambda 関数にルーティング |

### 認証

| コンポーネント | 実装 | 役割 |
|--------------|------|------|
| Lambda Authorizer | `src/dashboard_api/authorizer/` | JWT cookie を検証し、API Gateway の全エンドポイントを保護 |
| Auth Lambda 群 | `auth_login / auth_callback / auth_logout / auth_me` | Slack OAuth フロー（認可 URL 生成 → コールバック → JWT 発行 → セッション確認）を処理 |

Slack OAuth アプリの作成手順は `credential-setup.md` 6 を参照。
...
| `get_execution_events` | `GET /api/v1/executions/{id}/events` |
| `get_metrics_summary` | `GET /api/v1/metrics/summary` |
...
| `get_review_quality` | `GET /api/v1/metrics/review-quality` |
| `get_errors` | `GET /api/v1/errors` |
| `get_feedback_aggregation` | `GET /api/v1/metrics/feedback` |

各 Lambda は DynamoDB の `{prefix}-workflow-executions` / `{prefix}-events` テーブルを読み取り専用でクエリする。書き込みは行わない。

### DynamoDB イベントテーブル（`{prefix}-events`）
