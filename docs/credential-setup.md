...
**この節は Claude Code OAuth（`catch-expander/claude-oauth`）に関する説明です。Codex CLI OAuth は別の方式で管理します（後述 4.5 参照）。**
...
## 4.5 Codex CLI ChatGPT OAuth

レビュアーエージェントは Codex CLI（GPT-5.5）を使用する。認証情報は `~/.codex/auth.json` に配置する必要があり、Claude Code OAuth と同じ「Secrets Manager → ECS タスク起動時に復元」方式で管理する。
...
約 10 日（JWT の `exp` フィールドから確認）。Claude Code OAuth（8 時間）より大幅に長い。

### 認証情報の初回取得と登録

```bash
# 1. ローカル環境で Codex CLI にログイン（対話式）
codex

# 2. 認証ファイルの内容を Secrets Manager に登録
aws secretsmanager create-secret \
  --name "catch-expander/codex-auth" \
  --secret-string file://~/.codex/auth.json
```

### ECS タスクでの復元と書き戻し

`src/agent/main.py` の `_setup_codex_credentials` が起動時に `~/.codex/auth.json` を 0600 権限で作成する（`TASK_TYPE=feedback` パスでは実行されない）。
タスク終了時に `_writeback_codex_credentials` が内容変化を検知し、Secrets Manager に書き戻す（ベストエフォート）。書き戻し前に Secrets Manager の現在値を再取得し、並行タスクが既に更新していればスキップする（optimistic concurrency）。

### Token Refresher Lambda との関係

**Codex CLI の OAuth トークンは Token Refresher Lambda の管理対象外。**
Codex CLI はタスク実行中に内部で自動 refresh するため、Lambda による定期 refresh は不要。
Lambda は Claude OAuth のみを対象とする。
