...
> **進捗マーク同期 2026-04-30**: 実装は当時完了済みだったが tasklist のチェックボックスが追従していなかったため、commit `1eba566` で実装済みの T1〜T7 を `[x]` 化（T8.1〜T8.4 はスコープ外明記のため `[ ]` のまま）
...
### T2.2 `_collect_workspace_files` の実装 [x]
- design.md `3.3` の通り
...
### T2.3 `_classify_workspace_outcome` の実装 [x]
- design.md `3.4` の通り
...
### T2.4 `call_claude_with_workspace` の実装 [x]
- design.md `3.2` の通り
...
### T2.5 `_build_code_generation_prompt` の書き換え [x]
...
### T3.1 `_normalize_code_files_payload` 削除 [x]
- T1.2 で確認した呼び出し元が全て新方式に置き換わっていることを再確認
- 関数本体を削除

### T3.2 `_build_code_failure_diagnostics` 削除 [x]
- 同上
...
### T4.1 `Test_collect_workspace_files` 追加 [x]
...
### T4.2 `Test_classify_workspace_outcome` 追加 [x]
...
### T4.3 `Test_call_claude_with_workspace` 追加 [x]
