...
> **進捗マーク同期 2026-04-30**: 実装は当時完了済みだったが tasklist のチェックボックスが追従していなかったため、commit `947bc3b` / `4ac050f` で実装済みの T1〜T10 を `[x]` 化（T11.1〜T11.5 はスコープ外明記のため `[ ]` のまま）
...
### T1.2 `src/agent/main.py` の既存テストへの影響確認 [x]
...
### T2.3 `_post_slack_failure` の文面実装 [x]
...
## T3. ECS Task の credentials 書き戻し実装 [x]

### T3.1 `src/agent/main.py` に hash 関数と書き戻し関数を追加 [x]
...
### T3.3 既存テスト (`test_main.py`) への対応 [x]
- T1.2 で確認した影響箇所を修正
...
## T4. SAM template 更新 [x]

### T4.1 `TokenMonitorFunction` の IAM Policy 拡張 [x]
...
### T4.2 `AgentTaskRole` の IAM Policy 拡張 [x]
...
### T5.1 `tests/unit/token_monitor/test_app.py` 書き換え [x]
- 旧テストクラス（`TestIsTokenStale`, `TestPostSlackNotification`）を削除（旧関数が削除されるため）
- 新テストクラスを追加:

#### T5.1.1 `TestNeedsRefresh` [x]
- 残り 2h で False
- 残り 30min で True
- 失効済み（残り -1h）で True
- 残り ちょうど 1h ジャストで True（境界）

#### T5.1.2 `TestCallRefreshEndpoint` [x]
...
#### T5.1.3 `TestBuildUpdatedCredentials` [x]
...
#### T5.1.4 `TestLambdaHandlerFlow` [x]
...
### T5.2 `tests/unit/agent/test_main.py` に書き戻しテスト追加 [x]

#### T5.2.1 `test_writeback_skips_when_unchanged` [x]
- 起動時 hash と現在 hash が同じ → put_secret_value が呼ばれない

#### T5.2.2 `test_writeback_calls_put_when_changed` [x]
- credentials ファイルを書き換えた状態 → put_secret_value 呼ばれる

#### T5.2.3 `test_writeback_handles_missing_file` [x]
- credentials ファイル削除済み → WARN ログ + put_secret_value 呼ばれない、例外なし

#### T5.2.4 `test_writeback_swallows_put_exception` [x]
...
## T6. 初回 bootstrap 手順書作成 [x]

### T6.1 `.steering/20260425-auth-redesign-aipapers/initial-setup.md` を作成 [x]
...
### T8.1 `docs/architecture.md` の認証セクション更新 [x]
- 「Claude OAuth トークン管理」関連の記述を新方式に書き換え
- 「自動リフレッシュ Lambda」アーキテクチャを記述
- ECS 書き戻しの説明追加

### T8.2 `docs/functional-design.md` の Token Monitor 説明更新 [x]
- 「失効監視のみ」→「監視 + 自動延命」に変更
- 通知トリガーの変更を明記（stale → refresh 失敗時のみ）

### T8.3 `CLAUDE.md` の認証関連記述確認 [x]
