...
def lambda_handler(event: dict, context: object) -> dict:
    ...
    try:
        ...
        skipped = query_event_type(table, "oauth_refresh_skipped", from_ts, to_ts)
    ...
    last_skip_at = skipped[-1]["timestamp"] if skipped else None
    candidate_ts = [ts for ts in (last_refresh_at, last_failure_at, last_skip_at) if ts is not None]
    last_check_at = max(candidate_ts) if candidate_ts else None
