...
class TestAggregateIssueCategories:
    """Tier 2.2 P1: _aggregate_issue_categories の型ガードテスト

    LLM 応答が malformed なケースで AttributeError / TypeError を発生させず、
    値域外カテゴリは "other" にフォールバックすることを検証する。

    関連: memory/project_review_loop_recurring_patch_site.md
          obsidian/2026-04-29_codex-iterative-review-finds-multilayer-misses.md
    """

    def test_normal_5_categories(self):
        from orchestrator import _aggregate_issue_categories
        issues = [
            {"issue_category": "terraform_schema"},
            {"issue_category": "terraform_schema"},
            {"issue_category": "iam_action"},
            {"issue_category": "syntax"},
            {"issue_category": "api_version"},
            {"issue_category": "other"},
        ]
        assert _aggregate_issue_categories(issues) == {
            "terraform_schema": 2,
            "iam_action": 1,
            "syntax": 1,
            "api_version": 1,
    ...
    def test_missing_field_falls_back_to_other(self):
        from orchestrator import _aggregate_issue_categories
        issues = [{"item": "x", "severity": "error"}, {"description": "y"}]
        assert _aggregate_issue_categories(issues) == {"other": 2}

    def test_unknown_category_value_falls_back_to_other(self):
        from orchestrator import _aggregate_issue_categories
        issues = [{"issue_category": "custom_unknown"}, {"issue_category": "MissedTypo"}]
        assert _aggregate_issue_categories(issues) == {"other": 2}

    def test_non_list_input_returns_empty(self):
        from orchestrator import _aggregate_issue_categories
        assert _aggregate_issue_categories(None) == {}
        assert _aggregate_issue_categories("not-a-list") == {}
        assert _aggregate_issue_categories({"issues": []}) == {}
    ...
    def test_non_dict_issue_is_skipped(self):
        ...
        assert _aggregate_issue_categories(issues) == {"syntax": 1}

    def test_non_string_category_value_falls_back_to_other(self):
        from orchestrator import _aggregate_issue_categories
        issues = [
            {"issue_category": 42},
            {"issue_category": None},
        ...
        assert _aggregate_issue_categories(issues) == {"other": 3}
    ...
    def test_mixed_valid_and_invalid(self):
        ...
        assert _aggregate_issue_categories(issues) == {"iam_action": 2, "other": 2}


class TestReviewLoopMalformedGuards:
    """T1-2b: Codex 1 回目 P1 対応の malformed 応答耐性テスト。

    `_parse_claude_response` が非 dict (None / list / scalar) を返す場合や、
    issues が ["string"] / None の場合に AttributeError / TypeError で落ちないことを検証。

    既存 `TestReviewLoop` は `call_codex` ではなく `call_claude` をモックする
    pre-existing 不整合があるため、本クラスは `_aggregate_issue_categories` の
    境界に近いユニット単位で `_parse_claude_response` 結果のハンドリングを検証する。
    """

    def test_parse_claude_response_can_return_non_dict(self):
        # _parse_claude_response の挙動を確認: 非 dict (str / list / None) を返し得ること
        from orchestrator import _parse_claude_response
        # JSON 直値 (list)
        assert isinstance(_parse_claude_response('[1, 2, 3]'), list)
        # JSON 直値 (null)
        assert _parse_claude_response('null') is None
        # JSON 直値 (string)
        assert _parse_claude_response('"hello"') == "hello"

    def test_aggregate_handles_non_list_issues_from_malformed_review(self):
        # malformed parsed_review が非 dict だった場合のフォールバック後の集計
        from orchestrator import _aggregate_issue_categories
        # 想定: review_result = {"passed": False, "issues": [], ...} のフォールバック後
        assert _aggregate_issue_categories([]) == {}
